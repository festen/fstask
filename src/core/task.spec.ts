...
describe('task', () => {
  describe('create', () => {
    ...
    test('it should use function provided', async () => {
      ...
      const task = new Task({ name: 'Test', run: fn })

      expect(await task.run({ setOutput: () => {} } as any)).toBe(
        'TEST_EXECUTE',
      )
    })
  })
  ...
  describe('Sort', () => {
    test('it should take into account dependencies in task order', () => {
      ...
      const t1 = new Task({ name: 'task 1', dependencies: [t2] })
      const t4 = new Task({ name: 'task 4' })
      const t5 = new Task({ name: 'task 5' })
      const t6 = new Task({ name: 'task 6', dependencies: [t4, t5] })
      const t3 = new Task({ name: 'task 3', dependencies: [t1, t6] })
